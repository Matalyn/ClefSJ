...
def resultAddKey():
	...
	for copyNumber in range(copyNumberStart, copyNumberEnd):
		...
		conn.commit()
		cursor.execute("insert ignore into room values('"+keyNumber+"', '"+room+"')")
		conn.commit()
...
def resultReportClient():
	...
	client = cursor.fetchone()
	#lending
	cursor.execute("select l.keyNumber, l.copyNumber, r.room, cl.opens, l.lendDate, l.expectedReturnDate from room r, lent l, client c, clef cl where c.email='"+email+"' and l.email=c.email and l.keyNumber=cl.keyNumber and cl.keynumber=r.keynumber and l.copyNumber=cl.copyNumber")
	lend = cursor.fetchall()
	#lost
	cursor.execute("select lo.keyNumber, lo.copyNumber, r.room, cl.opens, lo.lendDate, lo.lossDate from room r, losshistory lo, client c, clef cl where c.email='"+email+"' and lo.email=c.email and lo.keyNumber=cl.keyNumber and cl.keynumber=r.keynumber and lo.copyNumber=cl.copyNumber")
	lost = cursor.fetchall()
	#return history
	cursor.execute("select re.keyNumber, re.copyNumber, r.room, cl.opens, re.lendDate, re.returnDate from room r, returnhistory re, client c, clef cl where c.email='"+email+"' and re.email=c.email and re.keyNumber=cl.keyNumber and cl.keynumber=r.keynumber and re.copyNumber=cl.copyNumber")
...
def addRoom():
	...
	cursor.execute("select distinct keyNumber from clef")
	keys = cursor.fetchall()
	return render_template('addRoom.html', keys = keys)
...
def resultAddRoom():
	...
	conn = mysql.connect()
	cursor = conn.cursor()
	cursor.execute("insert into room values('"+keyNumber+"', '"+room+"')")
	conn.commit()
	cursor.execute("select * from room where keyNumber='"+keyNumber+"'")
	rooms = cursor.fetchall()
	return render_template('resultAddRoom.html', rooms = rooms, keyNumber = keyNumber)
...
def updateRoom():
	...
	cursor.execute("select distinct keyNumber from room")
	keys = cursor.fetchall()
	return render_template('updateRoom.html', keys = keys)
...
def infoUpdateRoom():
	...
	cursor.execute("select room from room where keyNumber='"+keyNumber+"'")
	rooms = cursor.fetchall()
	return render_template('infoUpdateRoom.html', rooms = rooms, keyNumber = keyNumber)
...
def resultUpdateRoom():
	if not session.get('logged_in'):
		abort(401)
	keyNumber = request.form['keyNumber']
	oldRoom = request.form['oldRoom']
	newRoom = request.form['newRoom']
	conn = mysql.connect()
	cursor = conn.cursor()
	cursor.execute("update room set room='"+newRoom+"' where keyNumber='"+keyNumber+"' and room='"+oldRoom+"'")
	conn.commit()
	cursor.execute("select * from room where keyNumber='"+keyNumber+"'")
	rooms = cursor.fetchall()
	return render_template('resultUpdateRoom.html', rooms = rooms, keyNumber = keyNumber)
...
def reportKeysbyRoom():
	...
	cursor = mysql.connect().cursor()
	cursor.execute("select distinct room from room;")
	offices = cursor.fetchall()
...
def resultReportKeysbyRoom():
	...
	cursor = mysql.connect().cursor()
	cursor.execute("select c.keyNumber, c.copyNumber, c.opens from room r, clef c where r.keyNumber=c.keyNumber and r.room='"+room+"';")
	keys = cursor.fetchall()
