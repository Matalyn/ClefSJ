...
def resultReportKeysbyRoom():
    ...
    cursor = mysql.connect().cursor()
    cursor.execute("select c.keyNumber, c.copyNumber, o.description, c.status, l.email from unlocks u JOIN clef c JOIN opens o USING (keyNumber) LEFT OUTER JOIN lent l ON (l.keyNumber = c.keyNumber AND l.copyNumber = c.copyNumber) WHERE u.roomID=%s", (int(room),))
    keys = cursor.fetchall()
    cursor.execute("select * from room where id = %s", (int(room),))
    room = cursor.fetchone()
